            ...
            ItemScore(hit.getId, hit.getScore.toDouble,
              ...
              if (hit.getSource.containsKey("Price")) {
                ...
                if (!source_name.isEmpty) {
                  Some(source_name.get(0))
                } else Some("undefined")
              } else Some("undefined"),
              if (hit.getSource.containsKey("Image")) {
                val source_name = hit.getSource.get("Image").asInstanceOf[java.util.ArrayList[String]]
                if (!source_name.isEmpty) {
                  Some(source_name.get(0))
                } else Some("undefined")
              } else Some("undefined"),
              if (hit.getSource.containsKey("Name")) {
                val source_name = hit.getSource.get("Name").asInstanceOf[java.util.ArrayList[String]]
                if (!source_name.isEmpty) {
                  Some(source_name.get(0))
                } else Some("undefined")
              } else Some("undefined"),
              if (hit.getSource.containsKey("Author")) {
                val source_name = hit.getSource.get("Author").asInstanceOf[java.util.ArrayList[String]]
                if (!source_name.isEmpty) {
                  Some(source_name.get(0))
                } else Some("undefined")
              } else Some("undefined"),
              if (hit.getSource.containsKey("Url")) {
                val source_name = hit.getSource.get("Url").asInstanceOf[java.util.ArrayList[String]]
                if (!source_name.isEmpty) {
                  Some(source_name.get(0))
                } else Some("undefined")
              } else Some("undefined"),
              if (hit.getSource.containsKey("Slug")) {
                val source_name = hit.getSource.get("Slug").asInstanceOf[java.util.ArrayList[String]]
                if (!source_name.isEmpty) {
                  Some(source_name.get(0))
                } else Some("undefined")
              } else Some("undefined"))
